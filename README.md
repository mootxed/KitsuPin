...
> Поддерживаются только X11, KDE Plasma и Google Chrome.
...
- дедупликация по SHA-256 нормализованного содержимого + `domain_key`;
- позднее прикрепление Chrome metadata к карточке (late reconciliation, окно 5 с);
