...
- **Сессия Wayland (`XDG_SESSION_TYPE=wayland`)**:
  - В соответствии с архитектурой безопасности Wayland и KDE Plasma (KWin), прямой пассивный доступ фоновых приложений к буферу обмена без фокуса ограничен.
  - KitsuPin запускается в защищённом режиме **ограниченной поддержки** (Platform Capabilities). Приложение не падает и продолжает функционировать (интерфейс, системный трей, передача снимков страниц из браузерного расширения KitsuPin).
