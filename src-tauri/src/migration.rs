...
mod tests {
    ...
    fn test_scenario_a_directory_moved() {
        ...
        assert!(old_dir.join("pastily.sqlite3.migrated.bak").exists());
    }
    ...
    fn test_corrupted_legacy_row_rolls_back_transaction() {
        ...
        // Create old_db with clips table having bad column structure causing perform_legacy_merge to fail
        {
            ...
            conn.execute_batch(
                "CREATE TABLE clips (
                    id TEXT PRIMARY KEY,
                    bad_content_column TEXT NOT NULL,
                    domain TEXT,
                    created_at TEXT NOT NULL,
                    last_copied_at TEXT NOT NULL
                );
                INSERT INTO clips(id, bad_content_column, domain, created_at, last_copied_at)
